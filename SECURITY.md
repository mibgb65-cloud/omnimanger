...
- New accounts store a Worker-side PBKDF2 v2 authentication verifier. Legacy salted SHA-256 verifiers are upgraded after a successful login.
...
- Vault writes use revision checks to reduce accidental multi-device overwrites, but Cloudflare KV is still not a strongly consistent compare-and-swap store.
